            ...
            <ul className="flex-grow space-y-3 mb-8">
                {plan.features.map((feature, index) => (
                    <li key={index} className="flex items-start text-sm">
                        {feature.available ? (
                            <FaCheck className={`text-base mt-1 mr-3 flex-shrink-0 ${plan.recommended ? 'text-success-green' : 'text-success-green'}`} />
                        ) : (
                            <FaTimes className={`text-base mt-1 mr-3 flex-shrink-0 ${plan.recommended ? 'text-white/40' : 'text-border-grey'}`} />
                        )}
                        <span className={`${feature.available ? (plan.recommended ? 'text-white' : 'text-text-deep-grey') : (plan.recommended ? 'text-white/60' : 'text-text-deep-grey/60')}`}>
                            {feature.text}
                        </span>
                    </li>
                ))}
